...
test('loopback Gateway exposes bounded HTTP and signaling/tunnel WebSockets', async (t) => {
  ...
  const signalClosed = once(signal, 'close')
  await once(busy, 'open')
  await signalClosed
  busy.close(); await once(busy, 'close')
  const tunnelWhileSignal = new WebSocket('ws://127.0.0.1:' + port + '/tunnel/' + room)
  await once(tunnelWhileSignal, 'open')
  const replacedTunnel = new WebSocket('ws://127.0.0.1:' + port + '/tunnel/' + room)
  await once(replacedTunnel, 'open')
  tunnelWhileSignal.close(); await once(tunnelWhileSignal, 'close')
  replacedTunnel.close(); await once(replacedTunnel, 'close')
