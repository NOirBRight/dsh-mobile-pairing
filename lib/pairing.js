...
export class PairingOfferManager {
     ...
     */
    mint(mode, addr, room, pubkey, ice, hostName) {
        if (mode === 'direct') {
            if (room === null)
                throw new Error('direct pairing requires a signaling room');
    ...
    mintPublic(options) {
        ...
        if (!/^[0-9a-f]{32}$/.test(options.room))
            throw new Error('public endpoint pairing requires a 128-bit room');
    ...
    redeem(code) {
        ...
        if (pending === undefined || pending.claim !== undefined)
            return 'unknown';
     ...
     */
    claim(code, claimant, issue, room) {
        const pending = this.pending.get(code);
        if (pending === undefined)
            return { status: 'unknown' };
        if (pending.room !== undefined && pending.room !== room)
            return { status: 'unknown' };
        if (Date.now() > pending.expMs)
 ...
 */
export function buildOfferUrl(appUrl, offer) {
...
export function buildCompactPublicOfferUrl(appUrl, offer) {
    ...
    const payload = [
        ...
        ...(offer.hostName === undefined
            ? (offer.ice === undefined ? [] : [offer.ice])
            : [offer.ice ?? null, offer.hostName]),
    ];
 ...
 */
export function parseOfferUrl(url) {
    const hash = new URL(url).hash;
    if (!hash.startsWith('#offer='))
        return null;
    try {
        const payload = JSON.parse(Buffer.from(hash.slice('#offer='.length), 'base64url').toString());
        if (payload.v !== 1 && payload.v !== 2 && payload.v !== 3 && payload.v !== 4)
            return null;
        if (typeof payload.code !== 'string' || typeof payload.pubkey !== 'string')
            return null;
        if (payload.v === 2 && (payload.mode !== 'relay' || typeof payload.room !== 'string'))
            return null;
        if (payload.v === 3 && (payload.mode !== 'direct' || typeof payload.room !== 'string' ||
            !Array.isArray(payload.ice) || payload.ice.some(url => typeof url !== 'string' || !STUN_URL.test(url))))
            return null;
        if (payload.v === 4) {
            if (payload.mode !== 'public' || payload.protocol !== 1 || typeof payload.room !== 'string')
                return null;
            if (payload.endpointKind !== 'temporary' && payload.endpointKind !== 'custom')
                return null;
            try {
                if (new URL(payload.endpoint).protocol !== 'https:')
                    return null;
            }
            catch {
                return null;
            }
            if (payload.capabilities === null || typeof payload.capabilities !== 'object')
                return null;
            for (const key of ['browser', 'direct', 'tunnel', 'endpointRefresh']) {
                if (typeof payload.capabilities[key] !== 'boolean')
                    return null;
            }
            if (payload.ice !== undefined && (!Array.isArray(payload.ice) || payload.ice.some(url => typeof url !== 'string' || !STUN_URL.test(url))))
                return null;
        }
        return payload;
    }
