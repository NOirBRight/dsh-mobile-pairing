...
import { formatLoopbackAuthority, redactCookieDiagnostic } from "./dsh-cookie.js";
import WebSocket from 'ws';
import nacl from 'tweetnacl';
import { hostHandshake } from "./handshake.js";
import { WsRelayTransport } from "./host-transport.js";
...
function parseLoopbackTarget(path) {
...
function isHostAdministrationPath(pathname) {
 ...
 */
function startHostSession(transport, peerPub, options) {
    ...
    function sendWsPayload(id, data, binary) {
    ...
    // ── HTTP request path ────────────────────────────────────────────────────
    function onHttpReq(msg) {
        if (typeof msg.id !== 'string' || typeof msg.method !== 'string' || typeof msg.path !== 'string')
            return closeTransport(CLOSE_BAD_FRAME, 'bad http-req');
        if (typeof msg.headers !== 'object' || msg.headers === null)
            return closeTransport(CLOSE_BAD_FRAME, 'bad http-req headers');
        const target = parseLoopbackTarget(msg.path);
        if (target === null)
            return closeTransport(CLOSE_BAD_FRAME, 'bad http-req path');
        if (isHostAdministrationPath(target.pathname)) {
            if (state.requests.has(msg.id))
                return closeTransport(CLOSE_BAD_FRAME, 'duplicate http id');
        ...
        if (pending.size > MAX_BODY_BYTES)
            return refuseBody(msg.id);
        if (complete)
            void forwardRequest(msg.id);
    }
    function onHttpData(msg) {
        if (typeof msg.id !== 'string' || typeof msg.data !== 'string')
            return closeTransport(CLOSE_BAD_FRAME, 'bad http-data');
        const pending = state.requests.get(msg.id);
        if (pending === undefined)
            return closeTransport(CLOSE_BAD_FRAME, 'http-data for unknown id');
    ...
    async function forwardRequest(id) {
        ...
        if (options.waitCookie !== undefined) {
            try {
                ...
                if (waited !== undefined || cookie === undefined)
                    cookie = waited;
            }
            catch (error) {
        ...
        for (const [key, value] of Object.entries(pending.headers)) {
            ...
            if (key.toLowerCase() === 'cookie')
                continue; // phone cookie is not the DSH loopback cookie
            headers[key] = value;
        }
        ...
        const upstreamReq = request({ host: options.upstreamHost, port: options.upstreamPort, method: pending.method, path: pending.path, headers, timeout: 60_000, agent: false }, (upstreamRes) => collectResponse(id, upstreamRes));
    ...
    function collectResponse(id, res) {
        const chunks = [];
        let size = 0;
        let overflow = false;
        res.on('data', (chunk) => {
        ...
        res.on('end', () => {
            ...
            for (const [key, value] of Object.entries(res.headers)) {
                if (value === undefined || STRIPPED_RESPONSE_HEADERS.has(key))
                    continue;
                headers[key] = value;
            }
    ...
    function onWsOpen(msg) {
        if (typeof msg.id !== 'string' || typeof msg.path !== 'string')
            return closeTransport(CLOSE_BAD_FRAME, 'bad ws-open');
        const target = parseLoopbackTarget(msg.path);
        if (target === null)
            return closeTransport(CLOSE_BAD_FRAME, 'bad ws-open');
        if (state.bridges.has(msg.id) || state.closedBridges.has(msg.id))
            return closeTransport(CLOSE_BAD_FRAME, 'duplicate ws id');
        const id = msg.id;
        if (isHostAdministrationPath(target.pathname)) {
        ...
        void options.waitCookie().then((next) => { if (active && !state.closedBridges.has(id))
            start(next ?? currentCookie()); }, (error) => {
            const diagnostic = redactCookieDiagnostic(error);
            options.logger?.('DSH cookie wait failed: ' + diagnostic);
            if (active && !state.closedBridges.has(id))
                start(currentCookie());
        });
    }
    function onWsMsg(msg) {
        if (typeof msg.id !== 'string' || typeof msg.data !== 'string' || (msg.binary !== undefined && typeof msg.binary !== 'boolean'))
            return closeTransport(CLOSE_BAD_FRAME, 'bad ws-msg');
        const bridge = state.bridges.get(msg.id);
        if (bridge !== undefined) {
            const data = Buffer.from(msg.data, 'base64');
            if (bridge.readyState === WebSocket.OPEN)
                bridge.send(msg.binary === true ? data : data.toString('utf8'));
            return;
        }
        if (state.closedBridges.has(msg.id))
            return; // late frame for a closed bridge: normal race
        closeTransport(CLOSE_BAD_FRAME, 'ws-msg for unknown id');
    }
    function onWsClose(msg) {
        if (typeof msg.id !== 'string')
            return closeTransport(CLOSE_BAD_FRAME, 'bad ws-close');
        const bridge = state.bridges.get(msg.id);
        if (bridge !== undefined) {
    ...
    return {
        ...
        handleFrame(frame) {
            ...
            if (opened === null)
                return false; // not sealed under this session's key; the caller decides (relay: retry as handshake)
            if (opened.length > MAX_PLAINTEXT_BYTES)
                return closeTransport(CLOSE_TOO_LARGE, 'frame too large'), true;
            let msg;
            try {
                msg = JSON.parse(decoder.decode(opened));
            }
            catch {
            ...
            switch (msg.t) {
                case 'ping':
                    if (typeof msg.id !== 'string')
                        return closeTransport(CLOSE_BAD_FRAME, 'bad ping'), true;
                ...
                case 'ws-close':
                    ...
                    return true;
                default:
                    closeTransport(CLOSE_BAD_FRAME, 'unknown type ' + String(msg.t));
                    return true;
            }
        },
        teardown() {
 ...
 */
export function attachAuthenticatedTransport(transport, peerPub, options) {
    ...
    transport.onFrame((frame) => {
        ...
        if (typeof frame === 'string')
            return transport.close(CLOSE_BAD_FRAME, 'text frame');
        ...
        if (!session.handleFrame(frame))
            transport.close(CLOSE_BAD_FRAME, 'unsealable frame');
    });
    transport.onClose(() => {
        closed = true;
        session.teardown();
...
export function attachHandshakeTransport(transport, options) {
    const log = (msg) => options.logger?.(msg);
    let session = null;
    let closed = false;
    /** Replace the session after a successful (re-)handshake and send the ack. */
    function adoptSession(peerPub, ackFrame, resumed) {
        session?.teardown();
    ...
    transport.onFrame((frame) => {
        ...
        if (typeof frame === 'string')
            return transport.close(CLOSE_BAD_FRAME, 'text frame');
        if (session !== null && session.handleFrame(frame))
            return;
...
export function attachRelaySocket(socket, options) {
