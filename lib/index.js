...
import { createDshCookieAcquirer } from "./dsh-cookie.js";
...
export function apply(ctx, config) {
    ...
    function ensureRelayRoom(room, code) {
        ...
        if (previous?.relayUrl === live.relayUrl)
            return;
    ...
    function closeRelayRooms() {
        for (const campaign of relayCampaigns.values())
    ...
    ctx.effect(() => webServer.register({ kind: 'exact', path: '/pair/revoke', handler: async (req, res) => { if (req.method !== 'POST')
            ...
            return; const id = body.id; const room = typeof id === 'string' ? store.list().find(device => device.id === id)?.room : undefined; const revoked = typeof id === 'string' && store.revoke(id); if (revoked && room !== undefined) {
            relayCampaigns.get(room)?.connector.close();
    ...
    let launchUrl = undefined;
    const acquirer = createDshCookieAcquirer(() => launchUrl?.(), resolved.dshHost + ':' + String(resolved.dshPort));
    ctx.inject(['connection'], (connectionCtx) => {
        ...
        void acquirer.refresh().then((cookie) => {
            upstreamCookie = cookie;
            ctx.logger.warn('dsh-mobile-pairing: DSH loopback cookie acquired');
        }, (error) => { ctx.logger.warn('dsh-mobile-pairing: DSH cookie acquisition deferred: ' + String(error)); }).catch(() => { });
        return () => { };
    });
}
