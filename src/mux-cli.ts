...
import { readGatewayPort } from './gateway-port.ts'
...
export function parseMuxCliOptions(env: NodeJS.ProcessEnv): MuxCliOptions {
  ...
  return { bind: bind as '127.0.0.1' | '::1' | 'localhost', port, backends, backendHomes }
}

/** Merge static ports with ports published by each DSH Home. */
export function resolveMuxBackends(options: Pick<MuxCliOptions, 'backends' | 'backendHomes'>): number[] {
  const ports = [...options.backends]
  for (const home of options.backendHomes) {
    const published = readGatewayPort(home)
    if (published !== null && !ports.includes(published)) ports.push(published)
  }
  return ports
}
