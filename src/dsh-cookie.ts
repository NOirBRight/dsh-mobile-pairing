 ...
 *
 * Alpha.4 requires a signed browser cookie (authority-bound) even on loopback.
...
export function parseLoopbackAuthority(authority: string): { host: string; port: number } {
  ...
  if (authority === '::1') return { host: authority, port: 80 }
...
export function formatLoopbackAuthority(host: string, port: number): string {
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('dsh-mobile-pairing: loopback port must be an integer from 1 through 65535')
 ...
 */
export function redactCookieDiagnostic(error: unknown): string {
...
function requestOnce(host: string, port: number, path: string, cookie?: string, signal?: AbortSignal): Promise<{ status: number; setCookie?: string; location?: string }> {
...
export function createDshCookieAcquirer(
  ...
  async function fetchCookie(): Promise<string> {
    if (disposed) throw new Error('dsh-mobile-pairing: cookie acquirer is disposed')
  ...
  return {
    ...
    async refresh(force = false): Promise<string> {
      if (disposed) throw new Error('dsh-mobile-pairing: cookie acquirer is disposed')
      if (!force && cookie !== undefined && Date.now() - acquiredAt < maxAgeMs) return cookie
      if (force) { cookie = undefined; acquiredAt = 0 }
      if (inflight === null) {
